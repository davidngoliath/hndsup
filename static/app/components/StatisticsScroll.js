...
export default function StatisticsScroll({ statsRef }) {
    ...
    useEffect(() => {
        ...
        document.getElementById("horizontal-spacer2").style.height = `${totalScroll + window.innerWidth}px`;
        setWrap(totalScroll);
        setWindowsize(window.innerWidth);
                        ...
                        return (
                            <div className={statsStyles.statsSlide} key={index} ref={addPanel} id={item.id}>
                                <div className={statsStyles.statsSlideContent}>
                                    ...
                                    ></div>
                                    <img src={getAssetPath('/images/blueframe.svg')} alt="frame" className={statsStyles.blueFrame} />
